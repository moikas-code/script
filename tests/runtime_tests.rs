...
use script_lang::runtime::{self, Runtime, RuntimeConfig, ScriptRc};
use script_lang::script_assert;
use std::sync::Arc;
use std::thread;
    ...
    F: FnOnce(),
{
    ...
    if let Err(e) = runtime::initialize() {
        if !matches!(e, script_lang::runtime::RuntimeError::AlreadyInitialized) {
...
fn test_complex_object_graph() {
    with_runtime(RuntimeConfig::default(), || {
        ...
        let node3 = ScriptRc::new(Node { value: 3, next: None });
        let node2 = ScriptRc::new(Node { value: 2, next: Some(node3.clone()) });
        let node1 = ScriptRc::new(Node { value: 1, next: Some(node2.clone()) });
...
fn test_memory_limits() {
    ...
    with_runtime(config, || {
        ...
        let small = rt.memory().allocate(std::alloc::Layout::from_size_align(100, 8).unwrap());
        assert!(small.is_ok());
        
        // Should fail - exceeds limit
        let large = rt.memory().allocate(std::alloc::Layout::from_size_align(2048, 8).unwrap());
        assert!(large.is_err());
        
        // Clean up
        if let Ok(ptr) = small {
            rt.memory().deallocate(ptr, std::alloc::Layout::from_size_align(100, 8).unwrap());
        }
    });
}
