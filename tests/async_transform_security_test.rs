...
use script::lowering::async_transform::{transform_async_function, find_await_expressions};
use script::ir::{Module, Function, FunctionId, Type, Parameter, Visibility};
use script::parser::{Stmt, Expr, Location};
...
fn test_async_transform_await_limit() {
    ...
    for i in 0..150 {
        func.get_block_mut(entry).unwrap().add_instruction(
            ...
            Location { line: i as usize, column: 0 }
        );
    }
...
fn test_async_state_size_calculation() {
    ...
    block.add_instruction(
        script::ir::Instruction::CreateLocal {
            name: "int_var".to_string(),
            ty: Type::Named("i32".to_string()),
            value: script::ir::ValueId(1),
        },
...
fn test_async_transform_memory_safety() {
    ...
    // Add array creation
    block.add_instruction(
        script::ir::Instruction::CreateArray {
            element_ty: Type::Named("i32".to_string()),
            elements: vec![
                script::ir::ValueId(1),
                script::ir::ValueId(2),
                script::ir::ValueId(3),
    ...
    // Add struct creation
    block.add_instruction(
        script::ir::Instruction::CreateStruct {
...
fn test_async_transform_recursion_detection() {
    ...
    block.add_instruction(
        script::ir::Instruction::Call {
            function: script::ir::ValueId(0), // Would need resolution in real impl
...
fn test_async_transform_suspend_points() {
    ...
    let state_ids: Vec<u32> = transform_info.suspend_points
        .iter()
        .map(|sp| sp.state_id)
        .collect();
