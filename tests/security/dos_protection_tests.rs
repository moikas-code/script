...
mod type_inference_dos_tests {
    ...
    fn test_type_variable_explosion_protection() {
        ...
        assert!(last_error.is_some(), "Should return error when limit exceeded");
        
        if let Some(Error { kind: ErrorKind::SecurityViolation, message, .. }) = last_error {
            assert!(message.contains("Type variable limit exceeded"), 
                "Should indicate type variable limit: {}", message);
        } else {
    ...
    fn test_constraint_explosion_protection() {
        ...
        assert!(added_constraints < 60000, "Should limit constraint creation");
        assert!(last_error.is_some(), "Should return error when limit exceeded");
        
        if let Some(Error { kind: ErrorKind::SecurityViolation, message, .. }) = last_error {
            assert!(message.contains("Constraint limit exceeded"), 
                "Should indicate constraint limit: {}", message);
        } else {
            panic!("Should return SecurityViolation error");
        }
    }
    ...
    fn test_solving_iteration_protection() {
        ...
        match result {
            Err(Error { kind: ErrorKind::SecurityViolation, message, .. }) => {
                assert!(message.contains("iteration limit") || message.contains("timeout"), 
                    "Should indicate iteration limit or timeout: {}", message);
            }
            _ => {
        ...
        match result {
            Err(Error { kind: ErrorKind::SecurityViolation, message, .. }) => {
                assert!(message.contains("timeout"), 
                    "Should indicate timeout: {}", message);
            }
            Ok(_) => {
...
mod monomorphization_dos_tests {
    ...
    fn test_specialization_explosion_protection() {
        ...
        // Should hit the limit
        assert!(created_specializations < 1500, "Should limit specialization creation");
        assert!(last_error.is_some(), "Should return error when limit exceeded");
        
        if let Some(Error { kind: ErrorKind::SecurityViolation, message, .. }) = last_error {
            assert!(message.contains("specialization limit") || message.contains("Specialization limit"), 
                "Should indicate specialization limit: {}", message);
        } else {
            panic!("Should return SecurityViolation error");
        }
    }
    ...
    fn test_work_queue_explosion_protection() {
        ...
        if let Some(Error { kind: ErrorKind::SecurityViolation, message, .. }) = last_error {
            assert!(message.contains("work queue") || message.contains("queue size"), 
                "Should indicate work queue limit: {}", message);
        } else {
            panic!("Should return SecurityViolation error");
        }
    }
    ...
    fn test_monomorphization_timeout_protection() {
        ...
        match result {
            Err(Error { kind: ErrorKind::SecurityViolation, message, .. }) => {
                assert!(message.contains("timeout") || message.contains("time"), 
                    "Should indicate timeout: {}", message);
            }
            Ok(_) => {
        ...
        match result {
            Ok(analyzed) => {
                ...
                match lowering_result {
                    Ok(module) => {
                        assert!(lowering_elapsed < Duration::from_secs(30), 
                        ...
                        match mono_result {
                            Ok(_) => {
                                assert!(mono_elapsed < Duration::from_secs(30), 
