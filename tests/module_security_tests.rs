...
use script::module::{
    AuditConfig, ModuleChecksum, ModuleError, ModuleIntegrityVerifier, ModulePath,
    ModulePathSanitizer, PathSecurityValidator, ResourceMonitor, ResourceMonitorLimits,
    SecurityAuditLogger, SecurityEventCategory, SecuritySeverity,
    TrustLevel as IntegrityTrustLevel, VerificationRequirements,
};
use std::fs;
...
fn test_resource_limits() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Module limit exceeded"));
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Memory limit would be exceeded"));
...
fn test_audit_logging() {
    ...
    // Integrity violation
    logger
        .log_integrity_violation(module.clone(), "expected_hash", "actual_hash")
        .unwrap();

    // Resource exhaustion
    logger
        .log_resource_exhaustion(Some(module.clone()), "modules", 100, 150)
        .unwrap();
...
fn test_concurrent_operation_limits() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Concurrent operation limit"));
...
fn test_safe_display_path() {
    ...
    assert_eq!(
        validator.safe_display_path(&internal_path),
        "src/module.script"
    );
