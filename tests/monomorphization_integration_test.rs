...
use script::ir::instruction::{BinaryOp, Constant};
use script::ir::{Function, FunctionId, Instruction, IrBuilder, Module, Parameter, ValueId};
...
fn test_monomorphization_with_complex_types() {
    ...
    assert!(
        result.is_ok(),
        "Complex type monomorphization should succeed"
    );
    ...
    if let Some(specialized_func) = module.get_function_by_name("map_i32_string") {
        ...
        assert_eq!(
            specialized_func.params[0].ty,
            Type::Array(Box::new(Type::I32))
        );
        assert_eq!(
            specialized_func.return_type,
