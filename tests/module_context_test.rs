...
use script::module::{
    ImportPath, ModuleContext, ModuleContextStack, ModuleDependencyChain, ModulePath,
...
fn test_module_dependency_chain() {
    ...
    let utils_import = ImportPath::new("./utils").unwrap();
    ...
    let helpers_import = ImportPath::new("./helpers").unwrap();
...
fn test_module_context() {
    ...
    ctx.source_files
        .insert(module.clone(), PathBuf::from("test/module.script"));
    ...
    let failed_import = ImportPath::new("missing").unwrap();
    ctx.record_resolution(
        failed_import,
        None,
        span,
...
fn test_module_context_stack() {
    ...
    stack.push(ctx1);
    assert_eq!(
        stack.current().unwrap().current_module.to_string(),
        "module1"
    );
...
fn test_visibility_context() {
    ...
    vis_ctx.record_exports(
        module1.clone(),
        vec!["public_fn".to_string(), "PublicType".to_string()],
    );
...
fn test_module_context_error_formatting() {
    ...
    context
        .source_files
        .insert(module.clone(), PathBuf::from("app/main.script"));
