...
use script::module::{
    ModulePath, TrustLevel, ModuleCapability, ModuleSecurityContext, ModuleSecurityManager,
    PathPattern, Permission, FileSystemPermission, NetworkPermission, HostPattern,
    ModulePermissions, PermissionManager, SandboxConfig, ModuleSandbox,
    ModuleCompilationPipeline, CompilationConfig, ModuleLoadContext,
};
...
fn test_trust_level_capabilities() {
    ...
    assert!(!TrustLevel::Sandbox.allows_capability(&ModuleCapability::NetworkConnect("example.com".to_string())));
...
fn test_module_security_context() {
    ...
    assert!(ctx.grant_capability(ModuleCapability::FileRead(PathBuf::from("/tmp"))).is_ok());
    ...
    assert!(ctx.check_capability(&ModuleCapability::FileRead(PathBuf::from("/tmp"))).is_ok());
    
    // Check capability should fail for non-granted capabilities
    assert!(ctx.check_capability(&ModuleCapability::FileWrite(PathBuf::from("/tmp"))).is_err());
...
fn test_module_security_manager() {
    ...
    assert!(manager.check_import_permission(&company_path, &std_path).is_ok());
    assert!(manager.check_import_permission(&user_path, &company_path).is_ok());
    
    // Untrusted modules cannot import system modules
    assert!(manager.check_import_permission(&user_path, &std_path).is_err());
}
...
fn test_module_permissions_sets() {
    ...
    assert!(trusted.contains(&Permission::FileSystem(FileSystemPermission::Read(PathPattern::Any))));
    assert!(trusted.contains(&Permission::Network(NetworkPermission::Connect(HostPattern::Any))));
    assert!(!trusted.contains(&Permission::FileSystem(FileSystemPermission::Write(PathPattern::Any))));
    
    // System should have everything trusted has plus more
    assert!(system.contains(&Permission::FileSystem(FileSystemPermission::Read(PathPattern::Any))));
    assert!(system.contains(&Permission::FileSystem(FileSystemPermission::Write(PathPattern::Any))));
    assert!(system.contains(&Permission::FileSystem(FileSystemPermission::Execute(PathPattern::Any))));
    assert!(system.contains(&Permission::Process(script::module::ProcessPermission::Spawn(
        script::module::ProcessPattern::Any
    ))));
}
...
fn test_module_compilation_pipeline_security() {
    ...
    assert!(pipeline.grant_module_capability(
        &module_path,
    ...
    permissions.permissions.insert(Permission::FileSystem(
        FileSystemPermission::Read(PathPattern::Prefix(PathBuf::from("/tmp")))
    ));
    
    assert!(pipeline.set_module_permissions(&module_path, permissions).is_ok());
}
...
fn test_cross_module_security_checks() {
    ...
    assert!(security_manager.check_import_permission(&system_module, &trusted_module).is_ok());
    assert!(security_manager.check_import_permission(&system_module, &untrusted_module).is_ok());
    
    // Trusted can import from trusted and untrusted, but not sandbox-to-non-sandbox rule doesn't apply
    assert!(security_manager.check_import_permission(&trusted_module, &system_module).is_ok());
    assert!(security_manager.check_import_permission(&trusted_module, &untrusted_module).is_ok());
    
    // Untrusted cannot import system modules
    assert!(security_manager.check_import_permission(&untrusted_module, &system_module).is_err());
    assert!(security_manager.check_import_permission(&untrusted_module, &trusted_module).is_ok());
    
    // Sandbox cannot import from non-sandbox modules
    assert!(security_manager.check_import_permission(&sandbox_module, &system_module).is_err());
    assert!(security_manager.check_import_permission(&sandbox_module, &trusted_module).is_err());
}