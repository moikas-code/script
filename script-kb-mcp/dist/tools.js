 ...
 */
export function createTools(_kbManager) {
    return [
        {
            name: 'kb_read',
 ...
 */
export async function executeTool(toolName, args, kbManager) {
    switch (toolName) {
        case 'kb_read': {
        ...
        case 'kb_update': {
            ...
            if (!await kbManager.exists(args.path)) {
                if (!kbManager.isValidCategory(args.path)) {
                    ...
                    throw new Error(`Invalid category. File must be in one of: ${categories.join(', ')}`);
                }
            }
 ...
 */
function extractStatusSummary(content) {
 ...
 */
function extractIssuesSummary(content) {
    const lines = content.split('\n');
    const issues = [];
...
function determineSeverity(section) {
