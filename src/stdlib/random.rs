...
pub fn random_impl(_args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    RNG.with(|rng| {
        Ok(ScriptValue::F32(rng.borrow_mut().gen::<f32>()))
    })
}
...
pub fn random_range_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    ...
    RNG.with(|rng| {
        Ok(ScriptValue::F32(rng.borrow_mut().gen_range(min..max)))
    })
}
...
pub fn random_int_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    ...
    RNG.with(|rng| {
        Ok(ScriptValue::I32(rng.borrow_mut().gen_range(min..=max)))
    })
}
...
pub fn random_bool_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    let probability = if args.is_empty() {
        ...
        return Err(RuntimeError::InvalidOperation(
            format!("random_bool expects 0 or 1 argument, got {}", args.len())
        ));
    };
    
    RNG.with(|rng| {
        Ok(ScriptValue::Bool(rng.borrow_mut().gen::<f32>() < probability))
    })
}
...
pub fn rng_new_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    let rng = if args.is_empty() {
        ...
        return Err(RuntimeError::InvalidOperation(
            format!("rng_new expects 0 or 1 argument, got {}", args.len())
        ));
    };
    
    let mut map = HashMap::new();
    map.insert("_rng".to_string(), ScriptValue::NativeData(Box::new(rng)));
...
pub fn rng_random_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("rng.random expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn rng_random_range_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 3 {
        return Err(RuntimeError::InvalidOperation(
            format!("rng.random_range expects 3 arguments, got {}", args.len())
        ));
    }
...
pub fn rng_random_int_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 3 {
        return Err(RuntimeError::InvalidOperation(
            format!("rng.random_int expects 3 arguments, got {}", args.len())
        ));
    }
...
pub fn shuffle_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("shuffle expects 1 argument, got {}", args.len())
        ));
    }
    ...
    match &args[0] {
        ...
        _ => Err(RuntimeError::InvalidOperation("shuffle expects an array".to_string())),
    }
}
...
pub fn pick_random_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("pick_random expects 1 argument, got {}", args.len())
        ));
    }
    ...
    match &args[0] {
        ScriptValue::Array(arr) => {
            if arr.is_empty() {
                return Err(RuntimeError::InvalidOperation("Cannot pick from empty array".to_string()));
            }
            
            let index = RNG.with(|rng| {
                rng.borrow_mut().gen_range(0..arr.len())
            });
            
            Ok(arr[index].clone())
        }
        _ => Err(RuntimeError::InvalidOperation("pick_random expects an array".to_string())),
    }
}
...
pub fn random_in_circle_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    let radius = if args.is_empty() {
        ...
        return Err(RuntimeError::InvalidOperation(
            format!("random_in_circle expects 0 or 1 argument, got {}", args.len())
        ));
    };
...
pub fn weighted_random_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 2 {
        return Err(RuntimeError::InvalidOperation(
            format!("weighted_random expects 2 arguments, got {}", args.len())
        ));
    }
    
    let items = match &args[0] {
        ScriptValue::Array(arr) => arr,
        _ => return Err(RuntimeError::InvalidOperation("weighted_random expects an array as first argument".to_string())),
    };
    
    let weights = match &args[1] {
        ScriptValue::Array(arr) => arr,
        _ => return Err(RuntimeError::InvalidOperation("weighted_random expects an array as second argument".to_string())),
    };
    
    if items.len() != weights.len() {
        return Err(RuntimeError::InvalidOperation("Items and weights arrays must have same length".to_string()));
    }
    ...
    for w in weights.iter() {
        total_weight += w.to_f32()?;
    }
    
    if total_weight <= 0.0 {
        return Err(RuntimeError::InvalidOperation("Total weight must be positive".to_string()));
    }
...
fn get_rng_from_object(_obj: &ScriptValue) -> RuntimeResult<&ScriptRng> {
    ...
    Err(RuntimeError::InvalidOperation("RNG object access not fully implemented".to_string()))
}
...
mod tests {
    ...
    fn test_random_range() {
        for _ in 0..100 {
            let result = random_range_impl(&[ScriptValue::F32(0.0), ScriptValue::F32(1.0)]).unwrap();
            match result {
                ScriptValue::F32(val) => {
