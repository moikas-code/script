...
pub(crate) fn result_unwrap_err_impl(args: &[ScriptValue]) -> Result<ScriptValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("unwrap_err expects 1 argument, got {}", args.len())
        ));
    }
...
mod tests {
    ...
    fn test_option_methods() {
        ...
        assert_eq!(some_val.unwrap_or(ScriptValue::I32(0)), ScriptValue::I32(10));
    ...
    fn test_result_creation() {
        ...
        let err_val = ScriptResult::err(ScriptValue::String(
            ScriptRc::new(ScriptString::from_str("error"))
        ));
