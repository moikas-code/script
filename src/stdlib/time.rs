...
impl Stopwatch {
    ...
    pub fn reset(&mut self) {
        self.start_time = if self.is_running { Some(Instant::now()) } else { None };
...
pub fn time_now_millis_impl(_args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    let elapsed = APP_START_TIME.elapsed().as_secs_f32() * 1000.0;
    Ok(ScriptValue::F32(elapsed))
}
...
pub fn time_delta_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 2 {
        return Err(RuntimeError::InvalidOperation(
            format!("time_delta expects 2 arguments, got {}", args.len())
        ));
    }
...
pub fn sleep_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("sleep expects 1 argument, got {}", args.len())
        ));
    }
    
    let millis = args[0].to_f32()?;
    if millis < 0.0 {
        return Err(RuntimeError::InvalidOperation("Sleep time must be non-negative".to_string()));
    }
...
pub fn stopwatch_new_impl(_args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    ...
    map.insert("_stopwatch".to_string(), ScriptValue::NativeData(Box::new(stopwatch)));
...
pub fn stopwatch_start_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("stopwatch.start expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn stopwatch_stop_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("stopwatch.stop expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn stopwatch_reset_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("stopwatch.reset expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn stopwatch_elapsed_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("stopwatch.elapsed expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn frame_timer_new_impl(_args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    ...
    let mut map = HashMap::new();
    map.insert("_timer".to_string(), ScriptValue::NativeData(Box::new(timer)));
    Ok(ScriptValue::Object(ScriptRc::new(map)))
}
...
pub fn frame_timer_update_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("frame_timer.update expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn frame_timer_delta_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("frame_timer.delta expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn frame_timer_fps_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("frame_timer.fps expects 1 argument (self), got {}", args.len())
        ));
    }
...
pub fn format_time_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("format_time expects 1 argument, got {}", args.len())
        ));
    }
    
    let total_seconds = args[0].to_f32()? as i32;
    if total_seconds < 0 {
        return Err(RuntimeError::InvalidOperation("Time must be non-negative".to_string()));
    }
    ...
    Ok(ScriptValue::String(ScriptRc::new(formatted)))
}
...
pub fn format_time_millis_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("format_time_millis expects 1 argument, got {}", args.len())
        ));
    }
...
pub fn measure_time_impl(args: &[ScriptValue]) -> RuntimeResult<ScriptValue> {
    if args.len() != 1 {
        return Err(RuntimeError::InvalidOperation(
            format!("measure_time expects 1 argument (function), got {}", args.len())
        ));
    }
