...
use crate::security::{SecurityManager, SecurityPolicy, SecurityViolation};
...
impl ModuleSecurityPolicy {
    ...
    pub fn get_effective_policy(&self, module: &ModulePath) -> &SecurityPolicy {
        self.module_policies.get(module).unwrap_or(&self.base_policy)
    }
    ...
    pub fn track_allocation(
        ...
        let module_usage = usage.entry(module.clone()).or_insert_with(ModuleResourceUsage::new);
    ...
    pub fn track_deallocation(&self, module: &ModulePath, resource: &str, amount: usize) {
        ...
        if let Some(module_usage) = usage.get_mut(module) {
            match resource {
                "memory" => module_usage.memory_allocated = module_usage.memory_allocated.saturating_sub(amount),
                "file_handle" => module_usage.file_handles = module_usage.file_handles.saturating_sub(1),
                "network_connection" => module_usage.network_connections = module_usage.network_connections.saturating_sub(1),
                "thread" => module_usage.threads_spawned = module_usage.threads_spawned.saturating_sub(1),
                _ => {}
            }
            module_usage.last_activity = Instant::now();
        }
    ...
    pub fn set_isolation_boundary(&mut self, boundary: ModuleIsolationBoundary) {
        self.isolation_boundaries.insert(boundary.module.clone(), boundary);
    }
...
impl ModuleSecurityEnforcer {
    ...
    pub fn register_module(&mut self, context: ModuleSecurityContext) {
        ...
        self.policy.set_module_policy(context.module_path.clone(), module_policy);
    ...
    pub fn enforce_operation(
        ...
        self.policy.check_resource_limits(module, context)
            .map_err(|violation| Error::new(
                ErrorKind::SecurityViolation(violation),
        ...
        match operation {
            "file_read" => {
                if let Some(path) = args.get(0) {
                    ...
                    context.check_capability(&capability)
                        .map_err(|e| Error::new(
                            ErrorKind::Module(e),
    ...
    pub fn check_module_call(
        &self,
        caller: &ModulePath,
        callee: &ModulePath,
    ) -> Result<(), Error> {
        let caller_context = self.contexts.get(caller)
            .ok_or_else(|| Error::new(
                ErrorKind::Module(crate::module::ModuleError::not_found(caller.to_string())),
                format!("No security context for calling module {}", caller),
            ))?;
        
        self.policy.check_cross_module_call(caller, callee, caller_context.trust_level)
            .map_err(|violation| Error::new(
                ErrorKind::SecurityViolation(violation),
