...
use crate::module::{
    ...
    ModuleResult, ResolvedModule, ModuleSecurityContext, ModuleSecurityManager, TrustLevel,
    ModuleContext, ModuleContextStack, PermissionManager, ModuleSandbox, SandboxConfig,
};
...
impl<'a> CrossModuleTypeValidator<'a> {
    ...
    fn validate_statement(
        ...
        match &stmt.kind {
            StmtKind::Function { params, ret_type, body, .. } => {
        ...
        if type_name.contains("::") {
            ...
            if parts.len() == 2 {
                ...
                if let Some(referenced_module) = self.find_module_by_name(module_name) {
                    ...
                    if !referenced_module.exports.type_definitions.contains_key(type_name) {
                        return Err(ModuleError::import_error(
                            current_module.to_string(),
                            format!("Type '{}' not exported by module '{}'", type_name, module_name)
                        ));
                    }
                } else {
                    return Err(ModuleError::import_error(
...
impl ModuleCompilationPipeline {
    pub fn new(
        ...
        let security_enforcer = Arc::new(Mutex::new(
            ModuleSecurityEnforcer::new(crate::security::SecurityPolicy::default())
        ));
    ...
    fn compile_single_module(
        ...
        if let Some(ctx) = self.context_stack.current_mut() {
            ctx.source_files.insert(module_path.clone(), resolved_module.file_path.clone());
        }
        ...
        // Extract exports using the complete symbol table
        let exports = self.extract_module_exports(&ast, final_symbol_table)?;
    ...
    fn parse_module(&self, module: &ResolvedModule) -> ModuleResult<Program> {
        let lexer = Lexer::new(&module.source).map_err(|e| {
            ModuleError::parse_error(module.path.to_string(), e.to_string())
        })?;
    ...
    fn import_named_symbol(
        ...
        if let Some(function_info) = exports.functions.get(source_name) {
            ...
            return Err(ModuleError::import_error("current", format!("Symbol '{}' not found in exports", source_name)));
        }
    ...
    fn add_type_symbol(
        ...
        match &type_info.kind {
            ...
            TypeDefinitionKind::Alias { target_type: _ } => {
                ...
                return Err(ModuleError::config_error("Type alias imports not yet supported"));
            }
        };
        ...
        match fields {
            ...
            crate::parser::EnumVariantFields::Tuple(types) => {
                let tuple_types: Vec<crate::types::Type> = types
                    .iter()
                    .map(|type_ann| self.type_ann_to_type(type_ann).unwrap_or(crate::types::Type::Unknown))
                    .collect();
    ...
    fn extract_module_exports(
        ...
        // Extract exports from AST statements
        for stmt in &ast.statements {
            self.extract_exports_from_statement(stmt, &mut exports)?;
        }
    ...
    fn extract_exports_from_statement(
        ...
        match &stmt.kind {
            StmtKind::Export { export } => {
                self.process_export_statement(export, exports)?;
            }
            StmtKind::Function {
                name,
                generic_params,
    ...
    fn process_export_statement(
        ...
        match export {
            ...
            ExportKind::Variable { name, type_ann, init } => {
                let type_info = if let Some(type_ann) = type_ann {
                    self.type_ann_to_type(type_ann)?
                } else {
            ...
            ExportKind::Declaration(stmt) => {
                ...
                self.extract_exports_from_statement(stmt, exports)?;
            }
        }
    ...
    fn create_function_signature(
        ...
        let param_info: Result<Vec<_>, _> = params
            ...
            .map(|param| {
                ...
                Ok((param.name.clone(), param_type))
            })
            .collect();
    ...
    fn type_ann_to_type(&self, type_ann: &crate::parser::TypeAnn) -> ModuleResult<crate::types::Type> {
        use crate::parser::TypeKind;
        ...
        let result = match &type_ann.kind {
            ...
            TypeKind::Generic { name, args } => {
                let arg_types: Result<Vec<_>, _> = args
                    .iter()
                    .map(|arg| self.type_ann_to_type(arg))
                    .collect();
            ...
            TypeKind::Tuple(types) => {
                let tuple_types: Result<Vec<_>, _> = types
                    .iter()
                    .map(|t| self.type_ann_to_type(t))
                    .collect();
    ...
    fn validate_cross_module_types(
        ...
        // Create a cross-module type validator
        ...
        for stmt in &ast.statements {
            validator.validate_statement(stmt, &module.path)?;
        }
    ...
    pub fn execute_sandboxed(
        ...
        let security_context = {
            ...
            security_manager.contexts.get(module_path)
        ...
        sandbox.execute_function(function_name, args)
    ...
    pub fn get_module_security_info(&self, module_path: &ModulePath) -> Option<ModuleSecurityContext> {
        let security_manager = self.security_manager.lock().unwrap();
    ...
    pub fn get_module_context(&self, module_path: &ModulePath) -> Option<&ModuleContext> {
        self.context_stack.stack().iter()
            .find(|ctx| &ctx.current_module == module_path)
    }

    /// Format an error with full module context
    pub fn format_error_with_context(&mut self, error: &ModuleError, module_path: &ModulePath) -> String {
        if let Some(ctx) = self.context_stack.current_mut() {
