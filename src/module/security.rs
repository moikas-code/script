...
impl TrustLevel {
    ...
    pub fn allows_capability(&self, capability: &ModuleCapability) -> bool {
        match (self, capability) {
            ...
            // Unknown trust level defaults to no capabilities
            (TrustLevel::Unknown, _) => false,
        }
    }
    ...
    pub fn resource_limits(&self) -> ResourceLimits {
        match self {
            ...
            TrustLevel::Unknown => ResourceLimits {
                max_memory: 10_000_000, // 10MB - conservative default
                max_cpu_time: 1_000,    // 1 second
                max_file_handles: 0,
                max_network_connections: 0,
                max_threads: 0,
...
impl ModuleSecurityManager {
    ...
    pub fn new() -> Self {
        ModuleSecurityManager {
            ...
            system_patterns: vec!["std.*".to_string(), "core.*".to_string()],
        }
    }
    ...
    pub fn check_import_permission(
        ...
        let importer_ctx = self.contexts.get(importer).ok_or_else(|| {
            ModuleError::security_violation(format!(
                "No security context for importing module {}",
                importer
            ))
        })?;

        let imported_ctx = self.contexts.get(imported).ok_or_else(|| {
            ModuleError::security_violation(format!(
                "No security context for imported module {}",
                imported
            ))
        })?;
        ...
        if importer_ctx.trust_level == TrustLevel::Sandbox {
            if imported_ctx.trust_level != TrustLevel::Sandbox {
                return Err(ModuleError::security_violation(format!(
                    "Sandboxed module {} cannot import from non-sandboxed module {}",
                    importer, imported
                )));
            }
        }
        ...
        {
            return Err(ModuleError::security_violation(format!(
                "Untrusted module {} cannot import system module {}",
                importer, imported
            )));
        }
...
mod tests {
    ...
    fn test_trust_level_capabilities() {
        ...
        assert!(
            TrustLevel::Untrusted.allows_capability(&ModuleCapability::FileRead(PathBuf::new()))
        );
        assert!(
    ...
    fn test_module_security_context() {
        ...
        assert!(ctx
            .grant_capability(ModuleCapability::FileRead(PathBuf::new()))
            .is_ok());
        assert!(ctx
        ...
        assert!(ctx
            .check_capability(&ModuleCapability::FileRead(PathBuf::new()))
            .is_ok());
        assert!(ctx
