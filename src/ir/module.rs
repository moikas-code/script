...
impl Module {
    ...
    pub fn remove_function(&mut self, func_id: FunctionId) -> Result<Function, Error> {
        let function = self.functions.remove(&func_id).ok_or_else(|| {
            Error::new(
                ErrorKind::ModuleError,
    ...
    pub fn remove_function_by_name(&mut self, name: &str) -> Result<Function, Error> {
        let func_id = self.function_names.remove(name).ok_or_else(|| {
            Error::new(
                ErrorKind::ModuleError,
    ...
    pub fn set_function_name(
        ...
        let function = self.functions.get_mut(&func_id).ok_or_else(|| {
            Error::new(
                ErrorKind::ModuleError,
                format!("Function ID {} not found", func_id),
            )
    ...
    pub fn clone_function_by_name(&self, name: &str) -> Result<Function, Error> {
        let func_id = self.function_names.get(name).ok_or_else(|| {
            Error::new(
                ErrorKind::ModuleError,
                format!("Function '{}' not found", name),
            )
    ...
    pub fn replace_function(
        ...
        if old_function.name != new_function.name {
            ...
            self.function_names
                .insert(new_function.name.clone(), func_id);
        }
...
mod tests {
    ...
    fn test_add_function_duplicate_id() {
        ...
        let func1 = Function::new(FunctionId(100), "func1".to_string(), vec![], Type::I32);

        let func2 = Function::new(FunctionId(100), "func2".to_string(), vec![], Type::I32);
    ...
    fn test_add_specialized_function() {
        ...
        assert_eq!(
            module.get_function_id("generic_func"),
            Some(FunctionId(101))
        );
    }
    ...
    fn test_set_function_name() {
        ...
        let func = Function::new(FunctionId(100), "old_name".to_string(), vec![], Type::I32);

        let func_id = module.add_function(func).unwrap();

        module
            .set_function_name(func_id, "new_name".to_string())
            .unwrap();
    ...
    fn test_function_queries() {
        ...
        let func2 = Function::new(FunctionId(101), "func2".to_string(), vec![], Type::String);
