...
pub fn lower_statement(lowerer: &mut AstLowerer, stmt: &Stmt) -> LoweringResult<()> {
    match &stmt.kind {
        StmtKind::Let { name, init, .. } => {
            lower_let(lowerer, name, init.as_ref())
        }
        ...
        StmtKind::Return(expr) => {
            lower_return(lowerer, expr.as_ref())
        }
        StmtKind::While { condition, body } => {
            lowerer.lower_while(condition, body)
        }
...
fn lower_let(
    ...
    if let Some(init_expr) = init {
        ...
        let ptr = lowerer.builder.build_alloc(ty.clone())
...
fn lower_return(
    ...
    let value = expr.map(|e| lowerer.lower_expression(e))
        .transpose()?;
...
mod tests {
    ...
    fn test_lower_let_with_init() {
        ...
        lowerer.builder.create_function("test".to_string(), vec![], crate::types::Type::Unknown);
    ...
    fn test_lower_return() {
        ...
        // Create a dummy function context
        lowerer.builder.create_function("test".to_string(), vec![], crate::types::Type::I32);
