use super::Type;
use crate::parser::{TypeAnn, TypeKind, Literal, BinaryOp, UnaryOp};
...
pub fn type_from_ast(type_ann: &TypeAnn) -> Type {
    match &type_ann.kind {
        ...
        TypeKind::Array(elem_type) => {
            Type::Array(Box::new(type_from_ast(elem_type)))
        },
...
mod tests {
    ...
    fn test_type_from_ast_basic() {
        ...
        assert_eq!(type_from_ast(&custom_ann), Type::Named("MyType".to_string()));
    }
    ...
    fn test_infer_literal_type() {
        ...
        assert_eq!(infer_literal_type(&Literal::String("hello".to_string())), Type::String);
    ...
    fn test_binary_op_arithmetic() {
        ...
        assert_eq!(
            binary_op_result_type(&Type::F32, &Type::F32, &BinaryOp::Multiply),
            Ok(Type::F32)
        );
    ...
    fn test_unary_op_result() {
        assert_eq!(unary_op_result_type(&Type::Bool, &UnaryOp::Not), Ok(Type::Bool));
        assert_eq!(unary_op_result_type(&Type::I32, &UnaryOp::Negate), Ok(Type::I32));
        assert_eq!(unary_op_result_type(&Type::F32, &UnaryOp::Negate), Ok(Type::F32));
        
        // Invalid operations
        assert!(unary_op_result_type(&Type::String, &UnaryOp::Not).is_err());
        assert!(unary_op_result_type(&Type::Bool, &UnaryOp::Negate).is_err());
    }
}