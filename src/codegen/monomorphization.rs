...
impl MonomorphizationContext {
    ...
    /// Create a new monomorphization context with custom resource limits
    ...
    /// Create a new monomorphization context for development (more permissive limits)
    pub fn for_development() -> Self {
        Self::with_resource_limits(ResourceLimits::development())
    }

    /// Create a new monomorphization context for testing (very permissive limits)
    ...
    pub fn initialize_from_semantic_analysis(
        ...
        for instantiation in generic_instantiations {
            ...
            if self
                .type_registry
                .get_struct(&instantiation.function_name)
                .is_some()
            {
            ...
            // Check if it's an enum instantiation
                ...
                .type_registry
                .get_enum(&instantiation.function_name)
                .is_some()
            {
    ...
    fn process_type_monomorphization(&mut self) -> Result<(), Error> {
        ...
        while let Some((struct_name, type_args)) = self.struct_work_queue.pop_front() {
            ...
            if self.processed_structs.contains(&key) {
                self.stats.struct_duplicates_avoided =
                    self.stats.struct_duplicates_avoided.saturating_add(1);
                continue;
            }
            ...
            if let Some(generic_struct) = self.type_registry.get_struct(&struct_name).cloned() {
                if generic_struct.generic_params.is_some() && !generic_struct.is_monomorphized {
                    ...
                    self.type_registry
                        .register_monomorphized_struct(mangled_name.clone(), specialized_struct);
                    self.stats.structs_monomorphized =
                        self.stats.structs_monomorphized.saturating_add(1);
                    ...
                    self.type_registry
                        .cache_instantiation(struct_name, type_args, mangled_name);
                }
            }
        }
        ...
        while let Some((enum_name, type_args)) = self.enum_work_queue.pop_front() {
            ...
            if self.processed_enums.contains(&key) {
                self.stats.enum_duplicates_avoided =
                    self.stats.enum_duplicates_avoided.saturating_add(1);
                continue;
            }
            ...
            if let Some(generic_enum) = self.type_registry.get_enum(&enum_name).cloned() {
                if generic_enum.generic_params.is_some() && !generic_enum.is_monomorphized {
                    ...
                    self.type_registry
                        .register_monomorphized_enum(mangled_name.clone(), specialized_enum);
                    self.stats.enums_monomorphized =
                        self.stats.enums_monomorphized.saturating_add(1);
                    ...
                    // Cache the instantiation
                    self.type_registry
                        .cache_instantiation(enum_name, type_args, mangled_name);
                }
            }
        }
    ...
    pub fn monomorphize(&mut self, module: &mut Module) -> Result<(), Error> {
        // Start resource monitoring for monomorphization phase
                ...
                for block in function.blocks().values() {
                    for (_, instruction_with_loc) in &block.instructions {
                        if let Some((callee, type_args)) =
                            self.extract_generic_call(&instruction_with_loc.instruction, module)
                        {
        ...
        // Process work queue with resource monitoring
        let mut iteration_count = 0;
        while let Some((function_name, type_args)) = self.work_queue.pop_front() {
            iteration_count += 1;

            // Check resource limits every iteration
            self.resource_monitor
                .check_phase_timeout("monomorphization")?;
            self.resource_monitor.check_total_timeout()?;
            self.resource_monitor
                .check_iteration_limit("monomorphization", 1)?;

            // Check specialization limits
            self.resource_monitor
                .check_specialization_limit(self.instantiated_functions.len())?;

            // Check work queue size
            self.resource_monitor
                .check_work_queue_size("function_queue", self.work_queue.len())?;
            ...
            if let Some(generic_function) =
                self.find_generic_function(&generic_functions, &function_name)
            {
                let specialized_function =
                    self.specialize_function(generic_function, &type_args)?;
                ...
                self.stats.functions_monomorphized =
                    self.stats.functions_monomorphized.saturating_add(1);

                // Mark as processed
                self.processed_functions.insert(format!(
                    "{}_{}",
                    function_name,
                    self.mangle_type_args(&type_args)
                ));
            }
        }
        ...
        // Complete resource monitoring for monomorphization phase
    ...
    fn resolve_types_with_inference(&mut self, module: &Module) -> Result<(), Error> {
        ...
        let generic_functions: Vec<_> = module
            .functions()
            .values()
                ...
                if let Some(name) = self.get_function_name(func, module) {
                    ...
                    if let Some(generic_function) = module.get_function(*func) {
                        if self.is_generic_function(generic_function) {
                            ...
                            let inferred_types =
                                self.infer_type_arguments(generic_function, args, ty, module);
    ...
    fn scan_for_type_instantiations(&mut self, _module: &Module) {
        ...
        if let Some(type_params) = self.extract_type_params(generic_function) {
            ...
            for param in type_params {
                if let Some(concrete_type) =
                    self.infer_type_parameter(&param, generic_function, args, return_type, module)
                {
    ...
    fn infer_type_parameter(
        ...
        if let Some(concrete_type) =
            self.match_type_parameter(param, &generic_function.return_type, return_type)
        {
    ...
    fn type_ann_to_type(&self, type_ann: &crate::parser::TypeAnn) -> Type {
        ...
        match &type_ann.kind {
            ...
            TypeKind::Array(inner) => Type::Array(Box::new(self.type_ann_to_type(inner))),
    ...
    fn type_to_type_ann(&self, ty: &Type) -> crate::parser::TypeAnn {
        ...
        let kind = match ty {
            ...
            Type::Array(inner) => TypeKind::Array(Box::new(self.type_to_type_ann(inner))),
    ...
    fn substitute_instruction_types(&self, instruction: &mut Instruction, env: &GenericEnv) {
        match instruction {
            ...
            _ => {
                // Instructions like Store, StoreField, Return, Branch, CondBranch, CreateClosure
    ...
    fn mangle_type(&self, type_: &Type) -> String {
        match type_ {
            ...
            Type::Struct { name, fields } => {
                // Struct types include field types in mangling for uniqueness
    ...
    fn update_calls_in_function(
        &self,
        function: &mut Function,
        module: &Module,
        type_substitutions: &HashMap<FunctionId, FunctionId>,
    ) -> Result<(), Error> {
        ...
        for block_id in block_ids {
            if let Some(block) = function.get_block_mut(block_id) {
                ...
                for (_, instruction_with_loc) in &mut block.instructions {
                    if let Instruction::Call { func, ty, .. } =
                        &mut instruction_with_loc.instruction
                    {
    ...
    fn create_call_substitution_map(
        &self,
        module: &Module,
        generic_to_specialized: &HashMap<String, Vec<(String, FunctionId)>>,
        ...
        for (mangled_name, mut specialized_function) in self.instantiated_functions.drain() {
            ...
            if let Err(e) = module.add_function(specialized_function) {
                return Err(Error::new(
                    ...
                    format!(
                        "Failed to add specialized function '{}': {}",
                        mangled_name, e
                    ),
                ));
            }
        ...
        for func_id in function_ids {
            ...
            if let Some(function) = module.get_function(func_id) {
                if self.function_needs_call_updates(function, module) {
                    ...
                    self.update_calls_in_function(
                        &mut modified_function,
                        module,
                        &substitution_map,
                    )?;
...
mod tests {
    ...
    use crate::parser::{EnumVariant, EnumVariantFields, StructField, TypeAnn, TypeKind};
    ...
    fn test_type_mangling() {
        ...
        assert_eq!(
            ctx.mangle_type(&Type::Array(Box::new(Type::I32))),
            "array_i32"
        );
        assert_eq!(
            ctx.mangle_type(&Type::Option(Box::new(Type::String))),
            "option_string"
        );
    }
    ...
    fn test_function_specialization() {
        ...
        let specialized = ctx
            .specialize_function(&generic_function, &type_args)
            .unwrap();
    ...
    fn parser_to_types_generic_params(
        params: crate::parser::GenericParams,
    ) -> crate::types::generics::GenericParams {
        use crate::types::generics::{GenericParams, TraitBound, TypeParam};
    ...
    fn test_struct_monomorphization() {
        ...
        ctx.type_registry
            .register_struct(generic_struct.clone())
            .unwrap();
        ...
        assert_eq!(
            specialized.fields[0].type_ann.kind,
    ...
    fn test_enum_monomorphization() {
        ...
        ctx.type_registry
            .register_enum(generic_enum.clone())
            .unwrap();
