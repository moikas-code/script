...
use super::async_runtime::{Executor, BoxedFuture, BlockingExecutor, ScriptFuture};
...
struct SecurePointerRegistry {
    ...
    valid_pointers: HashMap<*mut (), PointerInfo>,
...
impl SecurePointerRegistry {
    ...
    fn register_pointer<T>(&mut self, ptr: *mut T, type_name: &'static str) -> u64 {
        ...
        self.valid_pointers.insert(ptr as *mut (), info);
        id
    }

    /// Validate and remove a pointer from registry
    fn validate_and_remove<T>(&mut self, ptr: *mut T, expected_type: &str) -> Result<PointerInfo, SecurityError> {
        if ptr.is_null() {
            return Err(SecurityError::NullPointer);
        }

        let generic_ptr = ptr as *mut ();
        match self.valid_pointers.remove(&generic_ptr) {
            Some(info) => {
    ...
    fn is_valid_pointer<T>(&self, ptr: *mut T, expected_type: &str) -> bool {
        ...
        let generic_ptr = ptr as *mut ();
        self.valid_pointers.get(&generic_ptr)
    ...
    fn cleanup_expired(&mut self) {
        ...
        self.valid_pointers.retain(|_, info| {
            now.duration_since(info.created_at) < one_hour
        });
    }
}

/// Thread-safe global pointer registry
static POINTER_REGISTRY: std::sync::OnceLock<Arc<Mutex<SecurePointerRegistry>>> = std::sync::OnceLock::new();

fn get_pointer_registry() -> Arc<Mutex<SecurePointerRegistry>> {
    POINTER_REGISTRY.get_or_init(|| {
        Arc::new(Mutex::new(SecurePointerRegistry::new()))
    }).clone()
}
...
fn script_join_all_internal(
    ...
    use super::async_runtime::JoinAll;
