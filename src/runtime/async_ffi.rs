...
use super::async_runtime_secure::{Executor, BoxedFuture, BlockingExecutor, AsyncRuntimeError, Timer, JoinAll};
use super::value::Value;
use crate::security::async_security::{AsyncSecurityManager, AsyncSecurityConfig};
...
fn get_global_executor() -> Result<Arc<Mutex<Executor>>, SecurityError> {
    let executor = GLOBAL_EXECUTOR.get_or_init(|| {
        Executor::new()
    }).clone();
...
fn validate_future_pointer<T>(ptr: *mut T, type_name: &str) -> Result<NonNull<T>, SecurityError> {
    ...
    let manager = security_manager.lock().map_err(|_| {
...
/// Safely convert raw pointer to Box with comprehensive validation
...
fn script_block_on_timeout_impl(
    ...
    if timeout_ms > MAX_TIMEOUT_MS {
        return Err(SecurityError::AsyncFFIViolation {
            ...
            message: format!("Timeout {} ms exceeds maximum {} ms", timeout_ms, MAX_TIMEOUT_MS),
        });
    }
    ...
    let security_manager = get_security_manager()?;
    {
        ...
        manager.validate_ffi_call("script_block_on_timeout", &[future_ptr as usize, timeout_ms as usize])?;
    }
...
fn script_sleep_impl(millis: u64) -> Result<*mut BoxedFuture<()>, SecurityError> {
    ...
    if millis > MAX_SLEEP_MS {
        return Err(SecurityError::AsyncFFIViolation {
            ...
            message: format!("Sleep duration {} ms exceeds maximum {} ms", millis, MAX_SLEEP_MS),
        });
    }
...
#[no_mangle]
pub extern "C" fn script_join_all(futures_ptr: *mut Vec<BoxedFuture<Value>>, count: usize) -> *mut BoxedFuture<Vec<Value>> {
    // Comprehensive security validation and error handling
...
/// Internal implementation with proper error handling
fn script_join_all_impl(futures_ptr: *mut Vec<BoxedFuture<Value>>, count: usize) -> Result<*mut BoxedFuture<Vec<Value>>, SecurityError> {
