...
impl MemoryProfiler {
    ...
    fn record_deallocation(&self, size: usize, type_name: &str) {
        ...
        if let Some((&addr, _)) = tracker.active.iter().find(|(_, info)| {
            info.type_name == type_name && info.size == size
        }) {
    ...
    fn report_leaks(&self) {
        ...
        if !tracker.active.is_empty() {
            ...
            for info in tracker.active.values() {
                leaks_by_type.entry(info.type_name.clone()).or_default().push(info);
            }
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!("  Total bytes allocated: {} bytes\n", stats.allocations.total_bytes_allocated));
        report.push_str(&format!("  Total bytes freed: {} bytes\n", stats.allocations.total_bytes_deallocated));
        ...
        report.push_str(&format!("  Objects collected: {}\n", stats.gc_objects_collected));
